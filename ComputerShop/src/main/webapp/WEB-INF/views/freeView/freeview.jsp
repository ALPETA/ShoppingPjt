<%@page import="com.hardCarry.shopping.entity.join.BoardReplyUsersEntity"%>
					...
					success : function(res) {
						...
						if (data.length > 0) {
							for (var i = 0; i < data.length; i++) {
								...
								ReplySection += '<div align="right"><a href="updateReplyView?name='
										+ data[i]['r_seq']
										+ '&num='
										+ data[i]['r_uSeq']
										+ '">수정</a>&nbsp&nbsp<a href="deleteReply?id='
										+ data[i]['r_uSeq']
										+ '&num='
										+ data[i]['r_bSeq']
										+ '&board='
										+ data[i]['r_seq'] + '">삭제</a></div>';
								ReplySection += '</div>';
...
<body>
	...
	<div class="container">
				...
				>
					...
					<tr>
						...
						<td><textarea
								...
								class="form-control"
								readonly
							><%=FreeView.getB_content()%></textarea></td>
					</tr>
					<tr>
						<td colspan="2">
							<div align="right">
								<a
									class="btn btn-lg1 btn btn-light"
									href="freeupdate.do?name=<%=FreeView.getB_seq()%>&id=<%=FreeView.getU_id()%>"
								>수정</a> <a
									class="btn btn-lg1 btn btn-light"
									id="deleteBtn"
									href="delete?name=<%=FreeView.getB_seq()%>&id=<%=FreeView.getU_id()%>"
								>삭제</a>
							</div>
						</td>
					</tr>
		...
		<div class="row">
			<div class="col-md-6">
				<a>작성일 : <fmt:formatDate
						value="<%=FreeView.getB_date()%>"
						pattern="yyyy-MM-dd"
					/></a>
			</div>
			<div
